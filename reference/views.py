...
def repartition_genre():
    ...
    # Plot Répartition par genre et pays
    ax1 = genre_pays.plot(kind='bar', stacked=True, figsize=(10, 7))
    ax1.set_title('Répartition par genre et pays')
    ax1.set_xlabel('Pays de Résidence')
    ax1.set_ylabel('Nombre')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.show()

    # Plot Répartition par genre et ville
    ax2 = genre_ville.plot(kind='bar', stacked=True, figsize=(10, 7))
    ax2.set_title('Répartition par genre et ville')
    ax2.set_xlabel('Ville de Résidence')
...
def genre_categorie_pro():
    ...
    enre_categorie.plot(kind='bar', stacked=True, figsize=(10, 7))
...
def habitants_par_zone():
    ...
    # Debug: Print columns to ensure 'pays_residence' exists
    print("Colonnes du DataFrame:", df.columns)

    # Vérifiez si 'pays_residence' est dans les colonnes
    if 'pays_residence' in df.columns:
        habitants_pays = df['pays_residence'].value_counts()
        return habitants_pays
    else:
